...
import { ThemeProvider as ThemeProviderBase, ThemeReference } from '@fluentui-react-native/theme';
import { TextV1 as Text } from '@fluentui-react-native/text';
import type { ThemeOptions } from '@fluentui-react-native/theme-types';
import { useTheme } from '@fluentui-react-native/theme-types';

// `ThemeProvider`'s element type comes from a package built against a different @types/react
// than this app pins, which trips TS2786 ("cannot be used as a JSX component"). The runtime
// component is correct; cast to a plain component type for the tsc build.
const ThemeProvider = ThemeProviderBase as React.ComponentType<React.PropsWithChildren<{ theme: ThemeReference }>>;

// A standalone example screen — a chat pane that exercises Avatar, Text, Button and live
// theming. It is the app's home view (apps/storybook/src/index), rendered directly rather
// than as a Storybook story.
...
const ChatPane: React.FunctionComponent = () => {
...
const AppearanceBar: React.FunctionComponent<{ value: Appearance; onChange: (a: Appearance) => void }> = ({ value, onChange }) => (
  <View style={styles.toolbar}>
    {appearances.map((a) => {
      ...
      return (
        ...
        >
          <Text variant="caption1" style={selected ? styles.chipTextSelected : styles.chipText}>
            {a.label}
          </Text>
        </Pressable>
      );
...
export const ChatPaneDemo: React.FunctionComponent = () => {
  const [appearance, setAppearance] = React.useState<Appearance>('dynamic');
  const themeRef = React.useMemo(
    () => new ThemeReference(createFluentTheme({ paletteName: 'TaskPane', appearance })),
    [appearance],
  );
...
const styles = StyleSheet.create({
  ...
  toolbar: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 8, justifyContent: 'center' },
  chip: {
  ...
  chipSelected: { backgroundColor: '#0F6CBD', borderColor: '#0F6CBD' },
  chipText: { color: '#888' },
